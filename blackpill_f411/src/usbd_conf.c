 ...
 *
 * BlackPill F411: USB OTG FS with internal PHY on PA11/PA12.
 * Much simpler than H7's ULPI setup — just 2 GPIO pins + 48MHz clock from PLL Q.
 */
...
void HAL_PCD_MspInit(PCD_HandleTypeDef *pcdHandle)
{
    ...
    /* PA11 = USB_DM, PA12 = USB_DP: AF10, push-pull, very high speed */
...
{
    if (pdev->id != DEVICE_FS) return USBD_OK;
     ...
     * OTG_FS total FIFO = 320 words = 1280 bytes. */
    HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_FS, 0x80);     /* 128 words shared RX */
    HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 0, 0x40);  /* EP0 TX: 64 words */
    HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG_FS, 1, 0x80);  /* EP1 TX: 128 words */
